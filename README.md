...
Temos uma aplicação full-stack (docker/todo-app). Esta aplicação precisa ser conteinerizada para funcionar. Desenvolvi os arquivos de configuração para cada frente específica: `Front-end`, `Back-end` e, neste caso, para um aplicativo de `teste` que valida se as aplicações estão se comunicando.
